...
def distributed_centralized_ImageNet_loader(dataset, data_dir,
    ...
    if args.if_timm_dataset:
        # TODO not completed
        train_data_local, test_data_local = get_timm_loader(train_dataset, test_dataset, args)
    else:
        ...
        train_data_local = data.DataLoader(train_dataset, batch_size=train_bs , sampler=train_sam,
                            pin_memory=True, num_workers=args.data_load_num_workers)
        train_global = data.DataLoader(train_dataset, batch_size=train_bs , sampler=None,
                            pin_memory=True, num_workers=args.data_load_num_workers)

        test_data_local = data.DataLoader(test_dataset, batch_size=test_bs, sampler=test_sam,
                            pin_memory=True, num_workers=args.data_load_num_workers)
        test_global = data.DataLoader(test_dataset, batch_size=test_bs, sampler=None,
                            pin_memory=True, num_workers=args.data_load_num_workers)
    ...
    for client_index in range(world_size):
        # client_index, len(train_data_local), len(test_data_local)))
        data_local_num_dict[client_index] = len(train_dataset) // world_size
...
def load_partition_data_ImageNet(dataset, data_dir, partition_method=None, partition_alpha=None, 
    ...
    for client_index in range(client_number):
        if partition_alpha is None:
            if client_number == 1000:
                if dataset in ['ILSVRC2012', 'ILSVRC2012_hdf5']:
                    ...
                    dataidxs = 0.1 * client_index
                    data_local_num_dict = class_num_dict
                else:
                    raise NotImplementedError
            elif client_number == 100:
                if dataset in ['ILSVRC2012', 'ILSVRC2012_hdf5']:
                    dataidxs = [client_index * 10 + i for i in range(10)]
                    data_local_num_dict[client_index] = sum(class_num_dict[client_index + i] for i in range(10))
                elif dataset in ['ILSVRC2012-100']:
                    dataidxs = client_index
                    data_local_num_dict = class_num_dict
                else:
        ...
        # local_data_num = data_local_num_dict[client_index]
